...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Rol alumno = new Rol();
        alumno.agregarUnidadDePermiso(entrarAlAula);

        //Creo un rol profesor y le asigno 2 permisos y 1 Rol
        Rol profesor = new Rol();
        profesor.agregarUnidadDePermiso(entrarAlAula);
        profesor.agregarUnidadDePermiso(ponerNota);
        profesor.agregarUnidadDePermiso(alumno); //Le estoy agregando 1 Rol dentro de 1 Rol

        // Creo un usuario y le asigno 1 rol y 1 permiso
        Usuario usuario = new Usuario();
        usuario.agregarUnidadDePermiso(alumno);
        usuario.agregarUnidadDePermiso(pasarMolinete);
